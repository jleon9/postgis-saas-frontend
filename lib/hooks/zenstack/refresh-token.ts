...
export function useCheckRefreshToken<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; hashedToken?: string; revoked?: boolean; userId?: string }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
